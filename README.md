...
RAGAS가 검색·답변의 **품질**을 평가한다면, 이 평가는 **에이전트가 도구를 올바르게
운용하는지**를 측정한다. 4가지 유형의 질문 48개를 자동 실행하고 실행 기록(trace)을
기계적으로 채점한다:

- **툴 선택 정확도** — 질문 유형에 맞는 도구를 호출했고, 불필요한 도구는 부르지 않았는가
- **멀티스텝 성공률** — "문서 검색 → 코드 생성" 순서의 연쇄 호출이 제대로 이뤄졌는가
- **인용 형식 준수율** — 답변 끝의 `Sources:` 섹션이 인용 규칙을 지켰는가 (정규식 판정)

| 분면 (각 12문항) | 툴 선택 정확도 | 멀티스텝 성공률 | 인용 준수율 | 평균 스텝 | 1건 평균 비용 |
|---|---|---|---|---|---|
| ① docs_search 단독 | 100% | — | 91.7% | 2.0 | $0.0009 |
| ② code_generate 단독 (순수 파이썬) | 100% | — | — | 5.0 | $0.0020 |
| ③ 복합 멀티스텝 (검색→코드생성) | 83.3% | 83.3% | — | 5.0 | $0.0024 |
| ④ 도구 불필요 | 100% | — | — | 1.0 | $0.0004 |
| **전체 (48문항)** | **95.8%** | **83.3%** | **91.7%** | | |

실패 2건은 모두 "LangChain API 코드 작성인데 docs_search를 건너뛰고 code_generate만
반복" 패턴으로, [알려진 한계](#알려진-한계-phase-4-에이전트) (a)와 정확히 일치한다 —
문서로만 적어두었던 한계가 재현 가능한 수치로 확인된 것. (비용은 gpt-4o-mini 기준)
